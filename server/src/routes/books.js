...
router.get('/', async (req, res) => {
  ...
  const rows = await db.prepare(`
    SELECT b.*, p.name as "publisherName", p.logo as "publisherLogo", p.logoText as "logoText", p.logoColor as "logoColor"
...
router.get('/:pubSlug/:bookId', async (req, res) => {
  const book = await db.prepare(`
    SELECT b.*, p.name as "publisherName", p.logo as "publisherLogo", p.logoText as "logoText", p.logoColor as "logoColor", p.description as "publisherDesc", p.city, p.founded
    FROM books b LEFT JOIN publishers p ON b.publisherSlug = p.slug
